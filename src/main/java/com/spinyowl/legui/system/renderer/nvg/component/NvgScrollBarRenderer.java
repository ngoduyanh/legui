...
import static com.spinyowl.legui.component.optional.Orientation.VERTICAL;
...
public class NvgScrollBarRenderer extends NvgDefaultComponentRenderer<ScrollBar> {
  ...
  public void renderSelf(ScrollBar scrollBar, GLFWContext context, long nanovg) {
    createScissor(nanovg, scrollBar);
    {
      ...
      // draw scroll bar back
      {
        ...
        NvgShapes.drawRect(nanovg, scrollBarPos, scrollBarSize,
            scrollBar.getStyle().getBackground().getColor(),
  ...
  private void drawArrows(long nanovg, ScrollBar scrollBar, Vector2f pos, Vector2f size) {
    ...
    if (arrowsEnabled) {
...

      {
        ...
        Vector4f secondArrowBounds = new Vector4f(arrow2pos.x, arrow2pos.y, arrowBgSize.x,
            arrowBgSize.y);

        NvgText.drawTextLineToRect(nanovg, firstArrowBounds, false, HorizontalAlign.CENTER,
            VerticalAlign.MIDDLE, fontSize,
            FontRegistry.MATERIAL_ICONS_REGULAR, firstArrowIcon, color);
        NvgText.drawTextLineToRect(nanovg, secondArrowBounds, false, HorizontalAlign.CENTER,
            VerticalAlign.MIDDLE, fontSize,
   ...
   */
  private void drawScrollButton(long nanovg, Vector2f pos, Vector2f size, ScrollBar scrollBar,
      float diff, float offset, boolean vertical) {
    ...
    float scrollPosAccordingToScrollBounds =
        diff + (scrollBarSize - barSize) * scrollBar.getCurValue() / valueRange;
    ...
    NvgShapes.drawRect(nanovg, scrollPos, scrollSize, scrollBar.getScrollColor(),
